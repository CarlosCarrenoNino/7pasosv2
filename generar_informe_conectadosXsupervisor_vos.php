...
$stmt = sqlsrv_query($conn, $sqlserver);
  ...
  $fila = 2;
	...
	$objPHPExcel->getProperties ()->setCreator("Descargue_X_SUPERVISOR_Voz")->setDescription("Descargue_X_SUPERVISOR_Voz");
	$objPHPExcel->setActiveSheetIndex (0);

		$objPHPExcel->getActiveSheet ()->setTitle ("Descargue_X_SUPERVISOR_Voz");       
 ...
 */		$objPHPExcel->getActiveSheet () -> setCellValue ('A1', 'CEDULA');
		...
		while($row=sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
			...
			$REG_CNOMBRE_ASESOR= "";
			$REG_DETALLE_2="";
			$CON_CSECPC_BOT=$row['CON_CSECPC_BOT'];
			...
			while($row_2=sqlsrv_fetch_array($stmt_2, SQLSRV_FETCH_ASSOC)){
				...
				$REG_CNOMBRE_ASESOR = $row_2['REG_CNOMBRE_ASESOR'];
				$REG_DETALLE_2 = $row_2['REG_DETALLE_2'];
