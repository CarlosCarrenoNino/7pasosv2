<?php
    ...
    class Modelo_grafico
    {
        ...
        function TraerDatosGraficosLlamadas()
        {
            date_default_timezone_set('America/Bogota');
            $fechaHoy=date('Y/m/d'); 
            $sql = "SELECT 'Paso ' + REG_CDETALLE0 as Pasos,  count(REG_CRUT_LABOR) as llamadas from TBL_AREGACTIVIDAD_BOT_VTR 
            WHERE  REG_DFEC_PASO0_BOT = '$fechaHoy' and REG_CDETALLE1= 'VOZ' group by REG_CDETALLE0";
             
            /* $sql = "SELECT 'Paso ' + REG_CDETALLE0 as Pasos,  count(REG_CRUT_LABOR) as llamadas from TBL_AREGACTIVIDAD_BOT_VTR 
            WHERE  REG_DFEC_PASO0_BOT = '$fechaHoy' group by REG_CDETALLE0"; */
